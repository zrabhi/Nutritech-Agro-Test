...
export const Header: FC<HeaderProp> = ({ name }) => {
  ...
  return (
    <div className="flex justify-between items-center p-4 sm:m-24 m-10">
      <div className="flex  flex-row">
        <img src={Picture.src} className="w-16 h-16" alt="image" />
      ...
      <Link href={` ${name === "Sign Up" ? "/" : "/login"}`}>
        <button
          className="sm:text-2xl sm:h-14 sm:w-36 w-28 h-8 text-[#ffff] text-white font-bold font-inter rounded-lg bg-Purple"
          onClick={() => {
